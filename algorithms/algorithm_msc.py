...
def algorithm_msc(raw_json_str, target_date_str):
    ...
    for event in events:
        ...
        if number_of_phase[-1]>=4 and any(word in status for word in ['CONSIGNEE', 'DISCHARGED FROM BARGE', 'AVAILABLE FOR DELIVERY', 'EMPTY RECEIVED', 'RETURNED']):
            number_of_phase.append(5)
        elif 4>=number_of_phase[-1]>=3 and any(word in status for word in ['CY', 'DISCHARG', 'UNLOAD', 'ARRIVAL', 'POSITIONING OUT']):
            number_of_phase.append(4)
        elif any(word in status for word in ['DEPARTURE', 'INTENDED TRANSIT', 'TRANSIT', 'TRANSSHIPMENT', 'DISCHARGED']):
            number_of_phase.append(3)
        elif any(word in status for word in ['LOAD', 'RECEIVED', 'STUFFED', 'POSITIONING IN']) and number_of_phase[-1]<=2:
