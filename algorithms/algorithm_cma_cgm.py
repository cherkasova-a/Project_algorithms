...
def algorithm_cma_cgm(raw_json_str, target_date_str):
    ...
    for event in events:
        ...
        if number_of_phase[-1]>=4 and any(word in status for word in ['GATE OUT', 'RETURN', 'CONSIGNEE', 'DELIVERY', 'FINAL']):
            number_of_phase.append(5)
        elif 4>=number_of_phase[-1]>=3 and any(word in status for word in ['DISCHARG', 'ARRIVAL', 'UNLOAD FULL FROM RAIL', 'IMPORT TRANSFER']):
            number_of_phase.append(4)
        elif any(word in status for word in ['DEPARTURE', 'TRANSIT', 'BOARD', 'ON RAIL', 'BARGE', 'TRAIN']):
            number_of_phase.append(3)
        elif any(word in status for word in ['GATE IN', 'EXPORT', 'LOAD', 'STUFFED', 'DEPOT']) and number_of_phase[-1]<=2:
