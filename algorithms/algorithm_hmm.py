...
def algorithm_hmm(raw_json_str, target_date_str):
    ...
    for event in events:
        ...
        if any(word in status for word in ["T/S", "TRANSIT", "DEPARTURE", "FEEDER DEPARTURE", "DISCHARGED"]):
            number_of_phase.append(3)
        elif number_of_phase[-1]>=4 and any(word in status for word in ["IMPORT", "GATE OUT", "DELIVERY", "RETURNED", "EMPTY RETURN", "DEVANNING", "CONSIGNEE"]):
            number_of_phase.append(5)
        elif 3<=number_of_phase[-1]<=4 and any(word in status for word in ["POD", "DISCHARGED", "FINAL POD", "ARRIVAL"]):
            number_of_phase.append(4)
        elif any(word in status for word in ["RELEASED", "GATE IN", "CY", "GATE IN TO RAIL", "LOAD", "UNLOAD"]) and number_of_phase[-1]<=2:
