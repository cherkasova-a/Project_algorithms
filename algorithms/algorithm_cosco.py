...
def algorithm_cosco(raw_json_str, target_date_str):
    ...
    for event in events:
        ...
        if number_of_phase[-1]>=3 and any(word in status for word in ['GATE-OUT', 'FINAL HUB' 'RETURNED', 'CONSIGNEE', 'DELIVERY']):
            number_of_phase.append(5)
        elif any(word in status for word in ['DISCHARG', 'ARRIVAL', 'UNLOAD', 'FINAL POD', 'LAST POD', 'RAILSPOT', 'OFF-RAIL']):
            number_of_phase.append(4)
        elif any(word in status for word in ['DEPARTURE', 'DEPARTED', 'T/S', 'BARGE', 'ON-RAIL', 'DESPATCH']):
            number_of_phase.append(3)
        elif any(word in status for word in ['LOAD', "CARGO RECEIVED", 'RECEIVED', 'STUFFED', 'DESPATCHED', 'POSITIONING IN', 'POL']) and number_of_phase[-1]<=2:
