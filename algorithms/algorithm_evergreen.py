...
def algorithm_evergreen(raw_json_str, target_date_str):
    ...
    for event in events:
        ...
        if any(word in status for word in ["PICK-UP BY MERCHANT HAULAGE", "RETURN", "DELIVERED", "GATE-OUT"]):
            number_of_phase.append(5)
        elif number_of_phase[-1]>=3 and any(word in status for word in ["WAITING", "DISCHARGED", "ARRIVAL", "READY FOR PICK-UP"]):
            number_of_phase.append(4)
        elif any(word in status for word in ["DEPARTURE", "IN TRANSIT"]):
            number_of_phase.append(3)
        elif any(word in status for word in ["LOAD", "WAITING", "TRANSSHIP CONTAINER", "PORT STORAGE"]) and number_of_phase[-1]<=2:
