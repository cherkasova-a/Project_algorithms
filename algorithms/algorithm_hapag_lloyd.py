...
def algorithm_hapag_lloyd(raw_json_str, target_date_str):
    ...
    for event in events:
        ...
        if number_of_phase[-1]>=4 and any(word in status for word in ['GATE OUT', 'RETURN', 'DELIVERY', 'GATE IN']):
            number_of_phase.append(5)
        elif 4>=number_of_phase[-1]>=3 and any(word in status for word in ['DISCHARG', 'ARRIVAL', 'ARRIVED', 'UNLOAD']):
            number_of_phase.append(4)
        elif any(word in status for word in ['DEPARTURE', 'DEPARTED', 'TRANSIT', 'TRANSSHIP']):
            number_of_phase.append(3)
        elif any(word in status for word in ['LOAD', 'GATE IN', 'ARRIVAL']) and number_of_phase[-1]<=2:
