...
def algorithm_concor(raw_json_str, target_date_str):
    ...
    for event in events:
        ...
        if any(word in status for word in ["CURRENTLY AT LOCATION"]) and number_of_phase[-1]>2:
            number_of_phase.append(5)
        elif any(word in status for word in ["GATE IN", "ARRIVED"]):
            number_of_phase.append(4)
        elif any(word in status for word in ["DEPARTURE", "DEPARTED", "FROM GATE"]):
            number_of_phase.append(3)
        elif any(word in status for word in ["GATE IN", "CURRENTLY AT LOCATION"]) and number_of_phase[-1]<=2:
