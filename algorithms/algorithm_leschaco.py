...
def algorithm_leschaco(raw_json_str, target_date_str):
    ...
    for event in events:
        ...
        if number_of_phase[-1]>=4 and any(word in status for word in ["GATE OUT", "DOOR DELIVERY", "GATE IN", "RETURN"]):
            number_of_phase.append(5)
        elif 3<=number_of_phase[-1]<=4 and any(word in status for word in ["DISCHARGED", "ARRIVAL", "UNLOADED", "CUSTOMS RELEASE IMPORT"]):
            number_of_phase.append(4)
        elif any(word in status for word in ["DEPARTURE", "TRANSIT"]) and number_of_phase[-1]<=4:
            number_of_phase.append(3)
        elif any(word in status for word in ["GATE OUT", "GATE IN", "LOAD", "RELEASE"]) and number_of_phase[-1]<=2:
