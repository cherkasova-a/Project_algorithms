...
def algorithm_kuehne_nagel(raw_json_str, target_date_str):
    ...
    for event in events:
        ...
        if number_of_phase[-1]>=4 and any(word in status for word in ["DELIVERY", "GATE OUT", "EMPTY GATE IN", "ARRIVAL (EMPTY)"]):
            number_of_phase.append(5)
        elif 3<=number_of_phase[-1]<=4 and any(word in status for word in ["UNLOAD", "ARRIVAL", "AVAILABLE", "READY FOR PICKUP"]):
            number_of_phase.append(4)
        elif any(word in status for word in ["DEPARTURE", "ON TRAIN", "ON VESSEL"]) and number_of_phase[-1]<=4:
            number_of_phase.append(3)
        elif any(word in status for word in ["PICKUP", "GATE IN", "GATE OUT", "LOAD"]) and number_of_phase[-1]<=2:
