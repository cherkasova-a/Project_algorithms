...
def algorithm_maersk(raw_json_str, target_date_str):
    ...
    for event in events:
        ...
        if number_of_phase[-1]>=4 and any(word in status for word in ['GATE-OUT-EMPTY', 'RETURN', 'CONSIGNEE', 'DELIVERY', 'FRTRELEA', 'CUSTOMER_GATE_IN']):
            number_of_phase.append(5)
        elif 4>=number_of_phase[-1]>=3 and any(word in status for word in ['DISCHARG', 'ARRIVAL', 'UNLOAD', 'DESTINATION', 'RAIL_ARRIVAL', 'RAILSPOT', 'OFF-RAIL']):
            number_of_phase.append(4)
        elif any(word in status for word in ['DEPARTURE', 'TRANSIT', 'TRANSSHIP', 'BARGE', 'ON-RAIL', 'RAIL_DEPARTURE']) and number_of_phase[-1]<=4:
            number_of_phase.append(3)
        elif any(word in status for word in ['LOAD', 'RAILSPOT', 'ON-RAIL', 'GATE-IN', 'RECEIVED', 'STUFFED', 'POSITIONING IN', 'CUSTOMER_GATE_OUT', 'FRTRELEA', 'GATE-OUT-EMPTY']) and number_of_phase[-1]<=2:
