...
def algorithm_oocl(raw_json_str, target_date_str):
    ...
    for event in events:
        ...
        if any(word in status for word in ['RETURNED', 'DELIVERY', 'DEVANNED', 'PLACEMENT']):
            number_of_phase.append(5)
        elif any(word in status for word in ['DISCHARGED', 'AVAILABLE', 'CUSTOMS', 'DERAMPED', 'POD']):
            number_of_phase.append(4)
        elif any(word in status for word in ['VESSEL', 'DEPARTED', 'DEPARTURE', 'TRANSIT', 'T/S']):
            number_of_phase.append(3)
        elif any(word in status for word in ['LOAD', 'RECEIVED BY CARRIER', 'RAMPED', 'SETTLED', 'POL']):
